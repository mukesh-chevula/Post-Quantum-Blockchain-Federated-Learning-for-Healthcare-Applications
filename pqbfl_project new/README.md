...
- **Off-chain (Python):** session establishment + **hybrid keying** (Kyber KEM + X25519) and **ratcheting** to derive per-round model keys.
- **Encrypted model exchange:** ChaCha20‑Poly1305 using per‑round model keys.
...
```bash
./pqbfl_project\ new/docker/docker-up.sh
```

Then open:
- UI: http://localhost:9501
- Chain RPC (host): http://localhost:9545

To stop and remove containers/volumes:

```bash
./pqbfl_project\ new/docker/docker-down.sh
```
...
- **Hybrid crypto:** Kyber512 (post‑quantum KEM) + X25519 (Curve25519).
- **Modern primitives (enhancements):** Ed25519 signatures, BLAKE3 hashing (falls back to SHA‑256 if unavailable).
